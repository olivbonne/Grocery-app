...
|--------|------------|---------|
| v0.40 | 2026-06-22 | Switching lists is now **instant** (in-app, no page reload) — which also means the **light/dark choice no longer resets** when you change lists. Removed the on-page "Clear buy-again history" link (the **Delete history** action lives in the buy-again category editor). Both the **Clear → Buy again** banner and the **Shopping-mode** banner now sit in the **fixed top region**, staying visible while scrolling. Collapse/Expand-all also toggles Buy again. The top region (input bar and above) is fixed. **Undo** button no longer wraps. **Lists** can be reordered (↑↓) and removed. Category heading **chips span the full width**. Fixed zoom **percentage** centering in Options. |
