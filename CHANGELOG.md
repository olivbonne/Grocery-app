...
|--------|------------|---------|
| v0.40 | 2026-06-22 | Collapse/Expand-all now also collapses/expands **Buy again**. The top region (input bar and everything above) is now **fixed** — it stays put while the list scrolls underneath (content is offset automatically). The **Clear → Buy again** banner sits in that fixed region so it's always visible. **Undo** button text no longer wraps. **Lists** menu now supports **reordering (↑↓) and removing** lists. Category heading **chips span the full screen width** (count right-aligned, "+" at the right edge). Fixed vertical centering of the zoom **percentage** in Options. |
