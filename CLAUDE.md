...
- **Model split (10-80-10):** plan and review on the **main thread (Opus 5, medium
  effort)**; run execution in the **`executor` subagent (Opus 5, low effort)** (medium-effort
  Opus for the thinking, low-effort Opus for the grind). Project settings default the
  main thread to `model: opus` / `effortLevel: medium`. See the RULE under **Workflow** below.
  (Standing user instruction — Opus 4.8 high/low → Sonnet 5 max/low → **Opus 5 medium/low**.)
- **RULE — execution runs in the `executor` subagent.** Reserve the main thread's
  medium-effort budget for planning and review; hand the heavy code-editing (the "80%") to
...
- **RULE — verification runs on the main thread (Opus 5, medium effort).** Behavioral
  ...
  shipping. The higher effort is spent where regressions are caught, not on the grind. Trivial
